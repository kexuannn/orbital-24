...
const Bookmarked = () => {
  ...
  <View key={pet.id} className="flex-row items-center border-b border-turqoise mb-2 h-9">
    ...
    <Text className={`flex-1 text-center font-plight p-2 h-full ${
      pet.status === "available" ? 'bg-green-500 text-white text-l ' :
      pet.status === "adopted" ? 'bg-red text-white text-l ' :
      pet.status === "pending adoption" ? 'bg-blue-500 text-xs text-white' :
      'bg-gray-200 text-black'  // Default color if none of the statuses match
    }`}>
      {pet.status}
    </Text>
  </View>
