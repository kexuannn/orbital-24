      ...
      <ScrollView>
        <View className="mt-10">
          ...
          <View className="justify-start ml-6">
            ...
            <Text className="text-turqoise font-pregular text-xl mt-2 underline mb-2">
              Posts:
            </Text>
            {userPosts.length > 0 ? (
              <FlatList
                data={userPosts}
                renderItem={({ item }) => (
                  <View key={item.id} className="bg-white mt-2 p-4 rounded-lg shadow-md mr-6">
                    <Image
              ...
              <Text className="text-darkBrown font-plight text-lg mt-2">No posts available</Text>
            )}
          </View>
        </View>
      </ScrollView>
